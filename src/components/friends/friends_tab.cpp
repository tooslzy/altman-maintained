...
static const char *presenceIcon(const string &p) {
	if (p == "InStudio") { return ICON_TOOL; }
	if (p == "InGame") { return ICON_CONTROLLER; }
	if (p == "Online") { return ICON_PERSON; }
	return "";
}
...
void RenderFriendsTab() {
	...
	auto isCurrentViewAccount
		= [&](const AccountData &a) { return a.id == g_viewAcctId && AccountFilters::IsAccountUsable(a); };
	...
	if (g_viewAcctId == -1 || std::none_of(g_accounts.begin(), g_accounts.end(), isCurrentViewAccount)) {
		...
		if (g_viewAcctId == -1) {
			...
			if (itFirst != g_accounts.end()) { g_viewAcctId = itFirst->id; }
		}
	}

	int currentAcctId = g_viewAcctId;
	auto it = find_if(g_accounts.begin(), g_accounts.end(), [&](auto &a) { return a.id == currentAcctId; });
		...
		if (BeginCombo("##AccountSelector", currentLabelStr.c_str())) {
			for (const auto &acc : g_accounts) {
				...
				if (Selectable(labelStr.c_str(), isSelected)) {
					if (!disabled) { g_viewAcctId = acc.id; }
				}
	...
	SameLine();
	if (Button((string(ICON_USER_PLUS) + " Add Friends").c_str())) { s_openAddFriendPopup = true; }
	EndDisabled();
	...
	if (BeginPopupModal("Add Friends", nullptr, ImGuiWindowFlags_AlwaysAutoResize)) {
		...
		TextUnformatted(
			"Enter one or more players, separated by commas or new lines. Each entry can be a username or a userId (formatted as id=000)."
		);
		PopTextWrapPos();
	...
	if (g_friendsViewMode == 1) {
			...
			for (size_t i = 0; i < reqsSnapshot.size(); ++i) {
				...
				if (BeginPopupContextItem("RequestRowContextMenu")) {
					...
					if (MenuItem("Copy User ID")) {
						string idStr = to_string(r.userId);
					...
					if (MenuItem("Accept Request")) {
						...
						Threading::newThread([uid, creds]() {
							...
							if (ok) {
								...
								if (g_selectedRequestIdx >= 0
									&& g_selectedRequestIdx < (int)g_incomingRequests.size()) {
									if (g_incomingRequests[g_selectedRequestIdx].userId == uid) {
										g_selectedRequestIdx = -1;
									}
								} else {
									g_selectedRequestIdx = -1;
		...
		if (!selectionValid) {
			...
			float requestLabelColumnWidth = GetFontSize() * 7.5f;
			{
				...
				requestLabelColumnWidth
					= (std::max)(requestLabelColumnWidth, mx + desiredTextIndent * 2.0f + GetFontSize());
			}
			if (BeginTable("RequestInfoTable", 2, tableFlags)) {
				...
				auto addRow = [&](const char *label, const string &value, bool wrapped = false) {
					...
					if (BeginPopupContextItem("CopyReqValue")) {
				...
				addRow(
					"Display Name:",
					!D.displayName.empty() ? D.displayName : (sel.displayName.empty() ? sel.username : sel.displayName)
				);
				// 2. Username
				...
				if (BeginPopupContextItem("CopyReqDesc")) {
					if (MenuItem("Copy")) { SetClipboardText(descStrReq.c_str()); }
					EndPopup();
				}
				EndChild();
				PopID();
			...
			if (openProfile) { OpenPopup("ProfileContext"); }
			...
			if (BeginPopup("ProfileContext")) {
				...
				if (!g_selectedAccountIds.empty()) {
					...
					auto itp = find_if(g_accounts.begin(), g_accounts.end(), [primaryId](const AccountData &a) {
						return a.id == primaryId;
					});
					if (itp != g_accounts.end()) {
						primaryCookie = itp->cookie;
						primaryUserId = itp->userId;
					}
				}
				...
				if (MenuItem("Profile")) {
					if (uid) {
						LaunchWebview(
							"https://www.roblox.com/users/" + to_string(uid) + "/profile",
							"Roblox Profile",
							primaryCookie,
							primaryUserId
						);
				...
				if (MenuItem("Friends")) {
					LaunchWebview(
						"https://www.roblox.com/users/" + to_string(uid) + "/friends",
						"Friends",
						primaryCookie,
						primaryUserId
					);
				...
				if (MenuItem("Favorites")) {
					LaunchWebview(
						"https://www.roblox.com/users/" + to_string(uid) + "/favorites",
						"Favorites",
						primaryCookie,
						primaryUserId
					);
				...
				if (MenuItem("Inventory")) {
					LaunchWebview(
						"https://www.roblox.com/users/" + to_string(uid) + "/inventory/#!/accessories",
						"Inventory",
						primaryCookie,
						primaryUserId
					);
	...
	if (g_friendsLoading.load() && g_friends.empty()) {
		...
		for (size_t i = 0; i < g_friends.size(); ++i) {
			...
			if (*icon) { label += icon; }
			label += (f.displayName == f.username || f.displayName.empty()) ? f.username
			...
			if (BeginPopupContextItem("FriendRowContextMenu")) {
				if (MenuItem("Copy Display Name")) { SetClipboardText(f.displayName.c_str()); }
				if (MenuItem("Copy Username")) { SetClipboardText(f.username.c_str()); }
				if (MenuItem("Copy User ID")) {
		...
		if (!g_unfriended.empty()) {
			...
			for (size_t i = 0; i < g_unfriended.size(); ++i) {
				...
				if (BeginPopupContextItem("FriendsLostEntryMenu")) {
					if (MenuItem("Copy Display Name")) { SetClipboardText(uf.displayName.c_str()); }
					if (MenuItem("Copy Username")) { SetClipboardText(uf.username.c_str()); }
					if (MenuItem("Copy User ID")) {
	...
	if (g_selectedFriendIdx < 0 || g_selectedFriendIdx >= static_cast<int>(g_friends.size())) {
		...
		if (D.id == 0 && g_friends[g_selectedFriendIdx].id != 0) {
			...
			float friendLabelColumnWidth = GetFontSize() * 7.5f; // start with a sensible minimum
			{
				...
				float mx = 0.0f;
				for (const char *lbl : labels) { mx = (std::max)(mx, CalcTextSize(lbl).x); }
				friendLabelColumnWidth
					= (std::max)(friendLabelColumnWidth, mx + desiredTextIndent * 2.0f + GetFontSize());
			}
			if (BeginTable("FriendInfoTable", 2, tableFlags)) {
				...
				auto addFriendDataRowInt
					= [&](const char *label, int value) { addFriendDataRow(label, to_string(value)); };
				...
				float reservedHeightBelowDescContent
					= spaceForBottomSpacingInCell + spaceForSeparator + spaceForButtons;
			...
			if (BeginPopup("ProfileContext")) {
				...
				if (MenuItem("Friends")) {
					LaunchWebview(
						"https://www.roblox.com/users/" + to_string(D.id) + "/friends",
						"Friends",
						primaryCookie,
						primaryUserId
					);
				...
				if (MenuItem("Favorites")) {
					LaunchWebview(
						"https://www.roblox.com/users/" + to_string(D.id) + "/favorites",
						"Favorites",
						primaryCookie,
						primaryUserId
					);
